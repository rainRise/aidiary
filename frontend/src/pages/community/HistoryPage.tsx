...
export default function HistoryPage() {
  ...
  useEffect(() => {
    setIsLoading(true)
    communityService
      .getViewHistory({ page, page_size: 20 })
      .then((res) => {
        setItems(res.items)
        setTotalPages(res.total_pages)
      })
      .catch(() => toast('加载浏览记录失败', 'error'))
      ...
      <header className="sticky top-0 z-50 backdrop-blur-xl border-b border-stone-200/70" style={{ background: 'rgba(248,245,239,0.85)' }}>
        <div className="max-w-2xl mx-auto px-4 sm:px-6">
          <div className="flex items-center h-14 gap-3">
            ...
            <h1 className="text-sm font-semibold text-stone-700">浏览记录</h1>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-2xl mx-auto px-4 sm:px-6 py-6">
        {isLoading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-stone-400 text-sm">还没有浏览记录</p>
          </div>
        ) : (
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-[10px] text-stone-300 ml-auto">{CIRCLE_ICONS[post.circle_id]}</span>
                  </div>
                  ...
                  <div className="flex items-center gap-5">
                    ...
                    <span className="flex items-center gap-1 text-[10px] text-stone-300 ml-auto">
                      <Clock className="w-3 h-3" /> 浏览于 {timeAgo(item.viewed_at)}
                    </span>
                  </div>
                </div>
              )
